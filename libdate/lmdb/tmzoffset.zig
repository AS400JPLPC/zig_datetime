...
var tbl_offset =  std.ArrayList(i128).init(allocTZ);
...
// std.debug.print("{s}  {d}\n",.{tbl_key.items[idx],tbl_offset.items[idx]});
        BaseTR.putInt(tbl_key.items[idx], tbl_offset.items[idx])  catch unreachable;
        idx += 1;
    }
...
//---------------------------------
pub fn readTimezone(keytmz : []const u8) i128 {
    ...
    const value = BaseTR.getInt(keytmz) catch unreachable;
