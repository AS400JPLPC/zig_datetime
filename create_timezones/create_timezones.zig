...
fn readFileTZ( file_TZ :[]const u8) ! void {
   ...
   try cDIR.deleteFile(file_TZ); 
    ...
    try src_lines.append("\n\n// Group Timezones");
    try src_lines.append("pub const lstgroup = std.ArrayList([] const u8).init(std.heap.page_allocator);");

    try src_lines.append("\npub fn addGroup() void {");
    group = "";
    for (array_tmzones.items) | tzone | {
        if ( !std.mem.eql(u8, tzone.group, "base")){
            if( !std.mem.eql(u8,group,tzone.group)) {
                const lstgroup = std.fmt.allocPrint(allocTZ,
                    "\tlstgroup.append(\"{s}\") catch unreachable;",.{tzone.group}) catch unreachable;
                try src_lines.append(lstgroup);
                group = tzone.group;
            }
        }    
    }
    try src_lines.append("\tlstgroup.append(\"base\") catch unreachable;");
    try src_lines.append("}");



    try src_lines.append("\n\n// list Timezones");
    try src_lines.append("pub const Tzone = struct {");
    try src_lines.append("\tgroup: []const u8,");
    try src_lines.append("\tzone: []const u8");
    try src_lines.append("};");
    try src_lines.append("\n\n");
    try src_lines.append("pub const lstTzone = std.ArrayList(Tzone).init(std.heap.page_allocator);");

    try src_lines.append("\npub fn addTzone() void {");
    for (array_tmzones.items) | tzone | {
                const lstTzone = std.fmt.allocPrint(allocTZ,
                    "\tlstTzone.append(Tzone{s}.group = \"{s}\" ,.zone =\"{s}\"{s}) catch unreachable;",
                    .{"{",tzone.group,tzone.key,"}"}) catch unreachable;

                try src_lines.append(lstTzone);
                group = tzone.group;
    }
    try src_lines.append("}");
